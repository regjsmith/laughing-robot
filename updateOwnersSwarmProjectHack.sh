...
# Script that checks if the users in the 'owners' array exist and outputs modified project key with just those users in the 'owners' array
    ...
    # Check if the owners array has changed and if it has print out p4 counter command to update the key
		...
		modifiedProjectKeyValue=$(p4 keys -e $projectKey | cut -d= -f2- | jq -c  '.owners |= $ARGS.positional' --args "${owners[@]}" )
	       ...
	       then
                    ...
                    echo "#   IMPORTANT: the command should be run in a *NIX shell like bash and NOT windwos cmd or PowerShell terminals"
